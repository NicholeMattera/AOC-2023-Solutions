    ...
    if (isMainThread) {
        ...
        for (var i = 0; i < almanac.seedsToPlant.length; i += 2) {
            ...
            worker.once('message', (location) => {
                ...
                if (workersFinished === workersCreated) {
                    console.log(`Part Two: ${lowestLocation}`)
                }
            });

            worker.postMessage({ almanac, seedStart: almanac.seedsToPlant[i], seedLength: almanac.seedsToPlant[i + 1] });
        }
    } else {
        parentPort.once('message', (message) => {
            parentPort.postMessage(almanac.getLowestLocationUsingRange(message.seedStart, message.seedLength))
        });
    }
})();
